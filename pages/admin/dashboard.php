...
$recent = [];
if ($recent_result) {
    while ($r = mysqli_fetch_assoc($recent_result)) $recent[] = $r;
}
?>
<!DOCTYPE html>
...
<div class="admin-layout">
    ...
    <aside class="admin-sidebar">
        ...
        <ul class="admin-sidebar-nav">
            <li><a href="<?= $base ?>pages/admin/dashboard.php" class="active"><span class="nav-icon">📊</span> Dashboard</a></li>
            <li><a href="<?= $base ?>pages/admin/proposals.php"><span class="nav-icon">📋</span> Kelola Proposal
                <?php if ($pending_proposals > 0): ?>
                    <span class="badge badge-pending" style="margin-left:auto;font-size:0.65rem;"><?= $pending_proposals ?></span>
                <?php endif; ?>
            </a></li>
            <li><a href="<?= $base ?>pages/admin/users.php"><span class="nav-icon">👥</span> Kelola Pengguna</a></li>
        </ul>
        <div class="admin-sidebar-section" style="margin-top:1.5rem;">Aksi Cepat</div>
        <ul class="admin-sidebar-nav">
            <li><a href="<?= $base ?>pages/tournament/create_admin.php"><span class="nav-icon">➕</span> Buat Turnamen</a></li>
    ...
    <main class="admin-content">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card" style="<?= $pending_proposals > 0 ? 'border-color:rgba(251,191,36,0.4);' : '' ?>">
                <div class="stat-label">Proposal Pending</div>
                <div class="stat-value <?= $pending_proposals > 0 ? '' : 'accent' ?>" style="<?= $pending_proposals > 0 ? 'color:#FBB024;' : '' ?>">
                    <?= number_format($pending_proposals) ?>
                </div>
                <?php if ($pending_proposals > 0): ?>
                    <a href="<?= $base ?>pages/admin/proposals.php" style="font-size:0.75rem;color:var(--accent);margin-top:0.25rem;display:block;">Review sekarang →</a>
                <?php endif; ?>
            </div>
        </div>

        <!-- Recent Tournaments -->
        <div class="section-title mb-2"><h2>Semua Turnamen</h2></div>
        <div class="table-wrap">
            <table>
                ...
                <tbody>
                    <?php if (empty($recent)): ?>
                        <tr><td colspan="6" style="text-align:center;color:var(--text-dim);padding:2rem;">Belum ada turnamen.</td></tr>
                    <?php else: ?>
                        <?php
                        $status_map = [
                        ...
                        ?>
                            <tr>
                                ...
                                <td style="color:var(--text-dim);font-size:0.82rem;"><?= date('d M Y', strtotime($t['created_at'])) ?></td>
                                <td>
