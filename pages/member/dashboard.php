...
$joined = [];
if ($joined_result) {
    while ($r = mysqli_fetch_assoc($joined_result))
        $joined[] = $r;
}
...
$managed = [];
if ($managed_result) {
    while ($r = mysqli_fetch_assoc($managed_result))
        $managed[] = $r;
}
...
$proposals = [];
if ($proposal_result) {
    while ($r = mysqli_fetch_assoc($proposal_result))
        $proposals[] = $r;
}
?>
<!DOCTYPE html>
    ...
    <div class="container page-wrap">
        ...
        <?php if (!empty($proposals)): ?>
            <div class="section-title mb-2">
                <h2>Status Proposal Kamu</h2>
            </div>
            <div
                style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:1rem;margin-bottom:2.5rem;">
                    ...
                    <div class="organizer-card">
                        ...
                        <div style="display:flex;gap:0.5rem;flex-wrap:wrap;">
                            ...
                            <?php if (in_array($t['status'], ['pending', 'open'])): ?>
                                <a href="/pages/tournament/edit.php?id=<?= $t['id'] ?>" class="btn btn-outline btn-sm">✏️ Edit</a>
                            <?php endif; ?>
                            <?php if (in_array($t['status'], ['open', 'ongoing'])): ?>
                                <a href="/pages/tournament/manage.php?id=<?= $t['id'] ?>" class="btn btn-primary btn-sm">Panel
                                    Kontrol →</a>
                            <?php endif; ?>
                        </div>
                    </div>
                ...
                <?php foreach ($joined as $t):
                    ...
                    <div class="tournament-card">
                        ...
                        <div class="tournament-card-footer">
                            ...
                            <a href="<?= $base ?>pages/tournament/detail.php?id=<?= $t['id'] ?>"
